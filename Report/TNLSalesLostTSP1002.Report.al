report 50208 "TNL Sales  Lost TSP1002"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layout/TNLSalesLostTSP1002.rdl';
    ...
    dataset
    {
        dataitem(SalesInvHeader; "Sales Invoice Header")
        {
            ...
            RequestFilterFields = "No.", "Sell-to Customer No.", "Posting Date", "Location Code";
            ...
            column(OrderDate_SalesInvoiceHeader; SalesInvHeader."Order Date")
            {
            }
            ...
            column(SalesInvoiceLine_GetCurrencyCode; SalesInvLines.GetCurrencyCode)
            {
            }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    dataitem(SalesInvLines; "Sales Invoice Line")
                    {
                        ...
                        DataItemLinkReference = SalesInvHeader;
                        DataItemTableView = SORTING("Document No.", "Line No.") WHERE("Posting Group" = CONST('N_PARTS'));
                        ...
                        column(VAT_SalesInvoiceLine; SalesInvLines."VAT %")
                        {
                        }
                        ...
                        column(Amount_ExclDisc; "Unit Price" * Quantity)
                        {
                        }
                        ...
                        column(QuantityDemanded_SalesInvoiceLine; SalesInvLines."Quantity Demanded")
                        {
                        }
                        column(Diff; SalesInvLines."Quantity Demanded" - Quantity)
                        {
                        }
                        ...
                        column(ItemCategoryCode_SalesInvoiceLine; SalesInvLines."Item Category Code")
                        {
                        }
                        ...
                        dataitem(DimensionLoop2; Integer)
                        {
                            ...
                            begin
                                ...
                                DimSetEntry2.SETRANGE("Dimension Set ID", SalesInvLines."Dimension Set ID");
                            end;
                        }
                        ...
                        trigger OnAfterGetRecord()
                        begin
                            //IF SalesInvLines.Quantity = 0 THEN
                            //  CurrReport.SKIP;
                    ...
                    dataitem(VATCounter; Integer)
                    {
                        ...
                        {
                            AutoFormatExpression = SalesInvLines.GetCurrencyCode;
                            AutoFormatType = 1;
                        }
...
#pragma warning restore AL0432
    ...
    begin
        NextEntryNo := 1;
        IF SalesInvLines."Shipment No." <> '' THEN
            IF SalesShipmentHeader.GET(SalesInvLines."Shipment No.") THEN
                EXIT(SalesShipmentHeader."Posting Date");

        IF SalesInvHeader."Order No." = '' THEN
            EXIT(SalesInvHeader."Posting Date");

        CASE SalesInvLines.Type OF
            SalesInvLines.Type::Item:
                GenerateBufferFromValueEntry(SalesInvLines);
            SalesInvLines.Type::"G/L Account", SalesInvLines.Type::Resource,
          SalesInvLines.Type::"Charge (Item)", SalesInvLines.Type::"Fixed Asset":
                GenerateBufferFromShipment(SalesInvLines);
            SalesInvLines.Type::" ":
                EXIT(0D);
        END;

        SalesShipmentBuffer.RESET;
        SalesShipmentBuffer.SETRANGE("Document No.", SalesInvLines."Document No.");
        SalesShipmentBuffer.SETRANGE("Line No.", SalesInvLines."Line No.");
        IF SalesShipmentBuffer.FIND('-') THEN BEGIN
            ...
            IF SalesShipmentBuffer.Quantity <> SalesInvLines.Quantity THEN BEGIN
                SalesShipmentBuffer.DELETEALL;
                EXIT(SalesInvHeader."Posting Date");
            END;
        END ELSE
    ...
    begin
        ...
        IF SalesInvLines.Type <> SalesInvLines.Type::Item THEN
            EXIT;
        ValueEntry.SETCURRENTKEY("Document No.");
        ValueEntry.SETRANGE("Document No.", SalesInvLines."Document No.");
        ValueEntry.SETRANGE("Document Type", ValueEntry."Document Type"::"Sales Invoice");
        ValueEntry.SETRANGE("Document Line No.", SalesInvLines."Line No.");
