report 50616 "Service - InvoiceX"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layout/ServiceInvoiceX.rdl';
    Caption = 'Service - Invoice';
    Permissions = TableData 7190 = rimd;

    dataset
    {
        dataitem(ServiceInvHeaderData; "Service Invoice Header")
        {
            ...
            column(CustomerOrderNo_ServiceInvoiceHeader; "Customer Order No.")
            {
            }
            ...
            column(TotalPaintcost_ServiceInvoiceHeader; "Total Paint cost")
            {
            }
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(BillToCustNo_ServInvHeader; ServiceInvHeaderData."Bill-to Customer No.")
                    {
                    }
                    column(BillToCustNo_ServInvHeaderCaption; ServiceInvHeaderData.FIELDCAPTION("Bill-to Customer No."))
                    {
                    }
                    column(PostingDate_ServInvHeader; FORMAT(ServiceInvHeaderData."Posting Date"))
                    {
                    }
                    column(VATNoText; VATNoText)
                    {
                    }
                    column(VATRegNo_ServInvHeader; ServiceInvHeaderData."VAT Registration No.")
                    {
                    }
                    column(DueDate_ServInvHeader; FORMAT(ServiceInvHeaderData."Due Date"))
                    {
                    }
                    ...
                    column(No1_ServiceInvHeader; ServiceInvHeaderData."No.")
                    {
                    }
                    column(ReferenceText; ReferenceText)
                    {
                    }
                    column(YorRef_ServInvHeader; ServiceInvHeaderData."Your Reference")
                    {
                    }
                    column(OrderNoText; OrderNoText)
                    {
                    }
                    column(OrderNo_ServInvHeader; ServiceInvHeaderData."Order No.")
                    {
                    }
                    ...
                    column(DocDate_ServInvHeader; FORMAT(ServiceInvHeaderData."Document Date", 0, 4))
                    {
                    }
                    column(PricesInclVAT_ServInvHeader; ServiceInvHeaderData."Prices Including VAT")
                    {
                    }
                    column(PricesInclVAT_ServInvHeaderCaption; ServiceInvHeaderData.FIELDCAPTION("Prices Including VAT"))
                    {
                    }
                    ...
                    column(PricesInclVAT1_ServInvHeader; FORMAT(ServiceInvHeaderData."Prices Including VAT"))
                    {
                    }
                    ...
                    {
                        DataItemLinkReference = ServiceInvHeaderData;
                        DataItemTableView = SORTING(Number);
                        ...
                        begin
                            IF NOT ShowInternalInfo THEN
                                CurrReport.BREAK;
                            FindDimTxt(ServiceInvHeaderData."Dimension Set ID");
                            SETRANGE(Number, 1, DimTxtArrLength);
                        end;
                    }
                    dataitem(ServiceInvLineData; "Service Invoice Line")
                    {
                        ...
                        column(BaseDisc_ServInvHeader; ServiceInvHeaderData."VAT Base Discount %")
                        {
                        }
                        ...
                        column(JobType_ServiceInvoiceLine; ServiceInvLineData."Job Type")
                        {
                        }
                        ...
                        {
                            AutoFormatExpression = ServiceInvHeaderData."Currency Code";
                            AutoFormatType = 1;
                        }
                        ...
                        dataitem("Service Shipment Buffer"; Integer)
                        {
                            ...
                            trigger OnPreDataItem()
                            begin
                                ServiceShipmentBuffer.SETRANGE("Document No.", ServiceInvLineData."Document No.");
                                ServiceShipmentBuffer.SETRANGE("Line No.", ServiceInvLineData."Line No.");
                        ...
                        dataitem(DimensionLoop2; Integer)
                        {
                            ...
                            begin
                                IF Number <= DimTxtArrLength THEN
                                    DimText := DimTxtArr[Number]
                                ELSE
                                    DimText := FORMAT(ServiceInvLineData.Type) + ' ' + AccNo;
                            end;
                            ...
                            begin
                                ...
                                FindDimTxt(ServiceInvLineData."Dimension Set ID");
                        ...
                        trigger OnAfterGetRecord()
                        begin
                            SETFILTER("Location Code", '<>%1', '131PWISOLO');
                            ...
                            VATAmountLine."VAT Base" := Amount + VATPaint;
                            VATAmountLine."Amount Including VAT" := "Amount Including VAT" + VATPaint;
                            VATAmountLine."Line Amount" := "Line Amount" + VATPaint;
                            IF "Allow Invoice Disc." THEN
                                VATAmountLine."Inv. Disc. Base Amount" := "Line Amount" + VATPaint;
                            VATAmountLine."Invoice Discount Amount" := "Inv. Discount Amount" + VATPaint;
                            VATAmountLine."VAT Clause Code" := "VAT Clause Code";
                            VATAmountLine.InsertLine;

                            TotalLineAmount2 += "Line Amount";
                            TotalAmount2 += Amount;
                            TotalAmountInclVAT2 += "Amount Including VAT";
                            TotalInvDiscAmount += "Inv. Discount Amount";
                            //VATPaint := 0.075 * ServiceInvHeaderData."Total Paint cost";
                            TypeInt := Type.AsInteger();

                            TotalLineAmount := TotalLineAmount2 + ServiceInvHeaderData."Total Paint cost";
                            TotalAmount := TotalAmount2 + ServiceInvHeaderData."Total Paint cost";
                            TotalAmountInclVAT := TotalAmountInclVAT2 + ServiceInvHeaderData."Total Paint cost" + VATPaint;
                        end;
                    ...
                    dataitem(VATCounter; Integer)
                    {
                        ...
                        {
                            AutoFormatExpression = ServiceInvLineData.GetCurrencyCode;
                            AutoFormatType = 1;
                        }
                    ...
                    dataitem(VATClauseEntryCounter; Integer)
                    {
                        ...
                        begin
                            ...
                            IF NOT VATClause.GET(VATAmountLine."VAT Clause Code") THEN
                                CurrReport.SKIP;
                            VATClause.TranslateDescription(ServiceInvHeaderData."Language Code");
                        end;
                    ...
                    dataitem(Total2; Integer)
                    {
                        DataItemTableView = SORTING(Number)
                                            WHERE(Number = CONST(1));
                        column(CustNo_ServInvHeader; ServiceInvHeaderData."Customer No.")
                        {
                        }
                        column(CustNo_ServInvHeaderCaption; ServiceInvHeaderData.FIELDCAPTION("Customer No."))
                        {
                        }
                ...
                begin
                    IF NOT CurrReport.PREVIEW THEN
                        ServiceInvCountPrinted.RUN(ServiceInvHeaderData);
                end;
            ...
            trigger OnAfterGetRecord()
            begin
                //CurrReport.LANGUAGE := LanguageVar.GetLanguageID("Language Code");
...
#pragma warning restore AL0432
#pragma warning disable AL0432
        VATAmountLine2: Record 290;
#pragma warning restore AL0432
        VATClause: Record 560;
        VATClause2: Record 560;
    ...
    begin
        NextEntryNo := 1;
        IF ServiceInvLineData."Shipment No." <> '' THEN
            IF ServiceShipmentHeader.GET(ServiceInvLineData."Shipment No.") THEN
                EXIT(ServiceShipmentHeader."Posting Date");

        IF ServiceInvHeaderData."Order No." = '' THEN
            EXIT(ServiceInvHeaderData."Posting Date");

        CASE ServiceInvLineData.Type OF
            ServiceInvLineData.Type::Item:
                GenerateBufferFromValueEntry(ServiceInvLineData);
            ServiceInvLineData.Type::"G/L Account", ServiceInvLineData.Type::Resource,
          ServiceInvLineData.Type::Cost:
                GenerateBufferFromShipment(ServiceInvLineData);
            ServiceInvLineData.Type::" ":
                EXIT(0D);
        END;
        ...
        IF ServiceShipmentBuffer.FIND('-') THEN BEGIN
            ...
            IF ServiceShipmentBuffer.Quantity <> ServiceInvLineData.Quantity THEN BEGIN
                ServiceShipmentBuffer.DELETEALL;
                EXIT(ServiceInvHeaderData."Posting Date");
            END;
        END ELSE
    ...
    begin
        ...
        ValueEntry.SETRANGE("Posting Date", ServiceInvHeaderData."Posting Date");
    ...
    begin
        ...
        ServiceInvoiceHeader.SETFILTER("No.", '..%1', ServiceInvHeaderData."No.");
        ServiceInvoiceHeader.SETRANGE("Order No.", ServiceInvHeaderData."Order No.");
        ...
        ServiceShipmentLine.SETRANGE("Order No.", ServiceInvHeaderData."Order No.");
    ...
    procedure AddBufferEntry(ServiceInvoiceLine: Record 5993; QtyOnShipment: Decimal; PostingDate: Date)
    begin
    ...
    begin
        ...
        IF NOT Customer.GET(ServiceInvHeaderData."Bill-to Customer No.") THEN
            EXIT;
