report 50285 "TNL Sales Invoice3"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layout/TNLSalesInvoice3.rdl';
    ...
    dataset
    {
        dataitem(SalesInvHeader; "Sales Invoice Header")
        {
            ...
            column(LogisticsApprovedBy_SalesInvoiceHeader; "Logistics Approved By")
            {
            }
            column(FinanceApprovedBy_SalesInvoiceHeader; "Finance Approved By")
            {
            }
...
#pragma warning restore AL0432
    ...
    begin
        ...
        URL := 'https://api.tdmsportal.com/api/qr/generateqrcodeV2?'
        + 'IRN=' + SalesInvHeader.IRN
        + '&supplier_key=tnl-787867676'
        ...
        if Client.get(URL, Response) then begin
            if Response.IsSuccessStatusCode() then begin
                ...
                // Store the image data in TempBlob
                TempBlob.CreateOutStream(OutS);
                CopyStream(OutS, InS);

                // Convert to Base64 for text field usage
                TempBlob.CreateInStream(InS);
                QRCode := Base64Convert.ToBase64(InS);
