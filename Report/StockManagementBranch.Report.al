...
{
    DefaultLayout = RDLC;
    RDLCLayout = './StockManagementBranch.rdlc';
    ...
    begin
          ...
          SETFILTER("Location Filter",Loc);
          CALCFIELDS("Sales (Qty.)","Negative Adjmt. (Qty.)",Inventory,"Qty. on Purch. Order","Qty. on Sales Order");
          TQS := "Sales (Qty.)";
          IF MaxPeriod <> 0 THEN
          Avsales := ROUND("Sales (Qty.)"/MaxPeriod,0.1);
          IF Avsales > 0 THEN BEGIN
            MIP := Avsales * ReOrPeriod;
            "Reorder Point" := ReOrPeriod * Avsales;
            "Safety Stock Quantity" := SafetyTime * Avsales;
           ...
           //SOQ := ((MaxPeriod * Avsales) - "Net Change" + "Qty. on Sales Order"); {SOQ is based on Maximum Period}
            SOQ := (MIP - "Net Change" + "Qty. on Sales Order"); /*suspended for the purpose of considering Maximum Period*/
             IF Considered THEN
