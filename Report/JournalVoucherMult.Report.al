report 50005 "Journal Voucher-Mult."
{
    DefaultLayout = RDLC;
    RDLCLayout = './JournalVoucherMult.rdlc';

    dataset
    {
        dataitem(DataItem5195; Table50103)
        {
            ...
            column(BankName_PaymentReceipt; "Payment/Receipt."."Bank Name")
            {
            }
            ...
            column(Payment_Receipt___Account_Description_; COPYSTR("Account Description", 1, 30))
            {
            }
            ...
            column(Transaction_DescriptionCaption; Transaction_DescriptionCaptionLbl)
            {
            }
            column(AccountNo_PaymentReceipt; "Payment/Receipt."."Account No.")
            {
            }
            ...
            column(BalanceAccountNo_PaymentReceipt; "Payment/Receipt."."Balance Account No.")
            {
            }
            column(BalAccDescription_PaymentReceipt; "Payment/Receipt."."Bal. Acc. Description")
            {
            }
            column(VoucherType_PaymentReceipt; "Payment/Receipt."."Voucher Type")
            {
            }
            column(ExternalDocumentNo_PaymentReceipt; "Payment/Receipt."."External Document No.")
            {
            }
            column(CapexOpexNo_PaymentReceipt; "Payment/Receipt."."Procurement No.")
            {
            }
            dataitem(DataItem1385; Table50104)
            {
                DataItemLink = No.=FIELD(No.),
                               Type=FIELD(Document Type),
                               Cash/Cheque=FIELD(Cash/Cheque);
                DataItemTableView = SORTING(Type,Cash/Cheque,No.,Line No.);
                column(AmountLCY_PaymentReceiptBalLine;"Payment/Receipt Bal. Line."."Amount (LCY)")
                {
                }
                ...
                column(Amount_PaymentReceiptBalLine;"Payment/Receipt Bal. Line.".Amount)
                {
                }
                column(ApplyEntry_PaymentReceiptBalLine;"Payment/Receipt Bal. Line."."Apply Entry")
                {
                }
                column(Transaction_Description_Line;"Transaction Description")
                {
                }
                ...
                begin
                    ...
                    AMOUNTINWORD := figure(TotalAmount,'NAIRA','KOBO');
            ...
            begin
                CompanyAddr[1] := CompanyData.Name;
                    ...
                    IF ("Payment/Receipt."."Account Type" IN ["Payment/Receipt."."Account Type"::"G/L Account",
                     "Payment/Receipt."."Account Type"::Bank]) AND ("Payment/Receipt."."Balance Account Type" IN
                     ["Payment/Receipt."."Balance Account Type"::Customer,"Payment/Receipt."."Balance Account Type"::Supplier])
                  THEN
                  BEGIN
                    CASE "Payment/Receipt."."Balance Account Type" OF
                       "Payment/Receipt."."Balance Account Type"::Customer :
                       IF Customer.GET("Payment/Receipt."."Balance Account No.") THEN
                           WITH Customer DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            CustomerAddr[3] := Address;
                            CustomerAddr[4] := "Address 2";
                            CustomerAddr[7] := City;
                            CustomerAddr[8] := Contact;
                            COMPRESSARRAY(CustomerAddr);
                           END;
                  "Payment/Receipt."."Balance Account Type"::Supplier :
                        IF vendor.GET("Payment/Receipt."."Balance Account No.") THEN
                           WITH vendor DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            CustomerAddr[3] := Address;
                            CustomerAddr[4] := "Address 2";
                            CustomerAddr[5] := "Address 3";
                            CustomerAddr[7] := City;
                            CustomerAddr[8] := Contact;
                            COMPRESSARRAY(CustomerAddr);
                           END;
                  "Payment/Receipt."."Balance Account Type"::Bank :
                           IF bank.GET("Payment/Receipt."."Balance Account No.") THEN
                           WITH bank DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            ...
                            COMPRESSARRAY(CustomerAddr);
                           END;
                  "Payment/Receipt."."Balance Account Type"::"Fixed Asset" :
                           IF fixed.GET("Payment/Receipt."."Balance Account No.") THEN
                           WITH fixed DO
                           BEGIN
                            CustomerAddr[1] := Description;
                            CustomerAddr[2] := "Description 2";
                            CustomerAddr[3] := "Location Code";
                            CustomerAddr[4] := "Responsible Employee";
                            COMPRESSARRAY(CustomerAddr);
                           END;
                           END;
                  END
                  ELSE
                  BEGIN
                    CASE "Payment/Receipt."."Account Type" OF
                       "Payment/Receipt."."Account Type"::Customer :
                       IF Customer.GET("Payment/Receipt."."Account No.") THEN
                           WITH Customer DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            ...
                            COMPRESSARRAY(CustomerAddr);
                           END;
                       "Payment/Receipt."."Account Type"::Staff :
                       IF Customer.GET("Payment/Receipt."."Account No.") THEN
                           WITH Customer DO
                           BEGIN
                            CustomerAddr[1] := Name;
                  ...
                  "Payment/Receipt."."Account Type"::Supplier :
                        IF vendor.GET("Payment/Receipt."."Account No.") THEN
                           WITH vendor DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            ...
                            COMPRESSARRAY(CustomerAddr);
                           END;
                  "Payment/Receipt."."Account Type"::Bank :
                           IF bank.GET("Payment/Receipt."."Account No.") THEN
                           WITH bank DO
                           BEGIN
                            CustomerAddr[1] := Name;
                            CustomerAddr[2] := "Name 2";
                            ...
                            COMPRESSARRAY(CustomerAddr);
                           END;
                  "Payment/Receipt."."Account Type"::"Fixed Asset" :
                           IF fixed.GET("Payment/Receipt."."Account No.") THEN
                           WITH fixed DO
                           BEGIN
                            CustomerAddr[1] := Description;
                            CustomerAddr[2] := "Description 2";
                ...
                IF ("Payment/Receipt."."Multiple Balance Account" = TRUE) OR ("Payment/Receipt."."Multiple Account" = TRUE) THEN
                 detatype := TRUE
                ELSE
                detatype := FALSE;
                IF "Payment/Receipt."."Amount (LCY)" = 0 THEN
                "Payment/Receipt."."Amount (LCY)" := "Payment/Receipt.".Amount;
                "Payment/Receipt.".MODIFY;



                IF NOT CurrReport.PREVIEW AND reprint THEN BEGIN
                  //"Payment/Receipt."."No. Printed" := "Payment/Receipt."."No. Printed" + 1;
                  "Payment/Receipt.".MODIFY;
                END;
            end;
            ...
            begin
                CompanyData.GET;
                "Payment/Receipt.".SETRANGE("Payment/Receipt."."No.",ReceiptNo);
            end;
        }
    }
    ...
    trigger OnPreReport()
    begin
        ReceiptNo := "Payment/Receipt.".GETRANGEMIN("Payment/Receipt."."No.");
        IF ReceiptNo = '' THEN
    ...
    var
        ...
        Delivery: Record "10";
        Payment: Record "3";
        SupplierAddr: array [8] of Text[30];
    ...
    begin
        ...
        IF ABS(fig) > 0 THEN
        BEGIN
        wordarray[1] := 'ONE'; wordarray[2]:= 'TWO'; wordarray[3] :='THREE'; wordarray[4] := 'FOUR';
        wordarray[5] := 'FIVE'; wordarray[6]:= 'SIX'; wordarray[7] :='SEVEN'; wordarray[8] := 'EIGHT';
        wordarray[9] := 'NINE'; wordarray[10]:= 'TEN'; wordarray[11] :='ELEVEN'; wordarray[12] := 'TWELVE';
        wordarray[13] := 'THIRTEEN'; wordarray[14]:= 'FOURTEEN'; wordarray[15] :='FIFTEEN';
        wordarray[16] := 'SIXTEEN'; wordarray[17] := 'SEVENTEEN'; wordarray[18]:= 'EIGHTEEN';
        wordarray[19] :='NINETEEN'; wordarray[20] := 'TWENTY';
        arrayval[1] := 'TEN'; arrayval[2] := 'TWENTY'; arrayval[3] := 'THIRTY'; arrayval[4] := 'FORTY'; arrayval[5] := 'FIFTY';
        arrayval[6] := 'SIXTY'; arrayval[7] := 'SEVENTY'; arrayval[8] := 'EIGHTY'; arrayval[9] := 'NINETY';
        arrayval[10] := 'HUNDRED'; arrayval[11] := 'THOUSAND'; arrayval[12] := 'MILLION'; arrayval[13] := 'BILLION';
        ...
        IF VALLENT > 15 THEN
          ...
          IF value5 > 0 THEN
          BEGIN                                             // unit and tens conversion begin
