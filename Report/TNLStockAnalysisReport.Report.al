...
{
    // //90915-YZZB2..90917-06082
    // Item."Unit Cost"
    // //Item."Unit Price"
    DefaultLayout = RDLC;
    RDLCLayout = './TNLStockAnalysisReport.rdlc';
    ...
    dataset
    {
        dataitem(DataItem8129; Table27)
        {
            ...
            begin
                Item.CALCFIELDS(Item."Sales (Qty.)",Item."Qty. on Purch. Order",Item.Inventory);
                ...
                StockCoverMonth := (Item."Sales (Qty.)"/PeriodDays);
                IF (StockCoverMonth<>0) THEN
                  StockCoverMonth := (Item.Inventory/StockCoverMonth)/30
                ELSE
                  StockCoverMonth := 0;
                SalesAnalysis(Item);
            end;
            ...
            begin
                ...
                Item.SETRANGE(Item."Date Filter",PeriodStarting,PeriodEnding);
            end;
        }
    }
    ...
    begin
        /*
        gFontSize := 8;
        //*GLAccount.SETRANGE(GLAccount."Default AccountCode",TRUE);
        //GLAccount.FIND('-');
        //AccountCode := GLAccount."No.";
        //AccountCode := '001';  //rem
        //GLEntry.SETRANGE(GLEntry."Posting Date",0D,WORKDATE);
        //GLEntry.FIND('+');
        //Period := acc."Starting Date";
        Bold := TRUE;
        Italic := TRUE;
        Underline := TRUE;
        //vclass.SETRANGE(vclass.Spare,FALSE);
        //VehicleRegister();
        
        //*XlWorkSheet.Range('A4:' + ExcelCol(c)+FORMAT(r+10)).Columns.AutoFit;
        //*XlWorkSheet.Range('D2:j2').Interior.ColorIndex := 10;
        XlApplication.Visible := TRUE;
        //If xlApplication.MailSession='' Then
        //xlApplication.MailLogon('erphq', 'adams', True);
        //XlWorkBook.SendMail('erphq@sevenup.com','sample fleet sent thru Navision-test',false);
        */
    ...
    begin
        /*    BEGIN
            UpdateCell(nr,1,Item."No.",NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,2,Item.Description,NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,3,Item."Shelf/Bin No.",NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,4,FORMAT(AverageCostLCY),NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,5,FORMAT(Item."Sales (Qty.)"),NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,6,FORMAT(Item.Inventory),NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,7,FORMAT(OnOrder),NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,8,FORMAT(StockCoverMonth),NOT Bold, NOT Italic, NOT Underline,10);
            UpdateCell(nr,9,FORMAT("Last Purchase Date"),NOT Bold, NOT Italic, NOT Underline,10);
            nr := nr + 1;
          END;
        */
