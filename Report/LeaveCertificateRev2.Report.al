...
{
    DefaultLayout = RDLC;
    RDLCLayout = './LeaveCertificateRev2.rdlc';

    dataset
    {
        dataitem(DataItem8187; Table50075)
        {
            ...
            column(Leave_Plan_Lines_Rev_2__Serial_No_; "Serial No")
            {
            }
            ...
            column(DataItem77; Please_note_that_failure_to_return_to_duty_on_the_above_date_without_accepted_excuse_will_be_treated_as_unauthorised_absence_Lbl)
            {
            }
            column(The_Finance___Accounts_Department_to_pay_the_sum_stated_on_item_13_above_on_the_next_salary_pay_day_Caption; The_Finance___Accounts_Department_to_pay_the_sum_stated_on_item_13_above_on_the_next_salary_pay_day_CaptionLbl)
            {
            }
            ...
            begin
                IF EmpRec.GET("Leave Plan Lines Rev 2"."Employee No.") THEN BEGIN
                    EmpName := EmpRec.FullName;
                    EmpGrp := EmpRec."Employee Group";
                    Designation := EmpRec.Designation;

                    IF Regrec.GET("Leave Plan Lines Rev 2"."Business Unit") THEN RegName := Regrec.Name;
                    IF EmpGrpRec.GET(EmpGrp) THEN BEGIN
                        EmpGrpRec.CALCFIELDS(EmpGrpRec."Net Pay Due");
                        // CSal := EmpGrpRec."Net Pay Due";
                    END;
                    ResumeDate := "Leave Plan Lines Rev 2"."Actual End Date";
                    REPEAT
                        ResumeDate := ResumeDate + 1;
                    UNTIL (NOT (GenPCode.IsHoliday(ResumeDate)));

                    IF LCatrec.GET("Leave Plan Lines Rev 2"."Leave Category") THEN LeaveDueto := LCatrec.Description;
    ...
    var
        ...
        Please_note_that_failure_to_return_to_duty_on_the_above_date_without_accepted_excuse_will_be_treated_as_unauthorised_absence_Lbl: Label 'Please note that failure to return to duty on the above date without accepted excuse will be treated as unauthorised absence.';
        The_Finance___Accounts_Department_to_pay_the_sum_stated_on_item_13_above_on_the_next_salary_pay_day_CaptionLbl: Label 'The Finance & Accounts Department to pay the sum stated on item 13 above on the next salary pay day.';
