report 50274 "MVO Stock 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './MVOStock2.rdlc';
    ...
    dataset
    {
        dataitem(DataItem8894; Table5802)
        {
            ...
            column(CostAmountEach;CostAmountEach)
            {
            }
            ...
            begin
                ...
                IF "Value Entry"."Inventory Posting Group" <> 'N_CARS' THEN
                  CurrReport.SKIP;
                ...
                ValuEntry.SETRANGE("Item No.","Value Entry"."Item No.");
                ValuEntry.SETFILTER("Posting Date",'%1..%2',0D,EndDate);
                ValuEntry.SETRANGE("Serial No.","Value Entry"."Serial No.");
                IF ValuEntry.FINDSET THEN REPEAT
                  IF ValuEntry."Item Ledger Entry Quantity" <> 0 THEN
                    iCount := iCount + 1;
                UNTIL ValuEntry.NEXT = 0;
                IF iCount <> 0 THEN
                  IF (iCount = 1) OR ((iCount MOD 2) = 1) THEN BEGIN
                    ItemLedgEntry.GET("Value Entry"."Item Ledger Entry No.");
                    EntryNo := ItemLedgEntry."Entry No.";
                    ValuEntry2.SETCURRENTKEY("Item No.","Posting Date","Item Ledger Entry No.");
                    ValuEntry2.SETRANGE("Item No.","Value Entry"."Item No.");
                    ValuEntry2.SETFILTER("Posting Date",'%1..%2',010113D,TODAY);//0D,EndDate);
                    ValuEntry2.SETRANGE("Item Ledger Entry No.",EntryNo);
                    IF ValuEntry2.FINDSET THEN BEGIN
                      ValuEntry2.CALCSUMS("Cost Posted to G/L");
                      CostAmountEach := ValuEntry2."Cost Posted to G/L";
                    END;
                  END ELSE
                    CurrReport.SKIP;
                ...
                IF ValuEntry2.FINDSET THEN BEGIN
                  ValuEntry2.CALCSUMS("Cost Posted to G/L");
                  CostAmount := ValuEntry2."Cost Posted to G/L";
                END;
            end;
            ...
            begin
                ...
                "Value Entry".SETFILTER("Posting Date",'%1..%2',0D,EndDate);
