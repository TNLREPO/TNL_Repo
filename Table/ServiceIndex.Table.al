table 50045 "Service Index"
{
    ...
    fields
    {
        ...
        {
            TableRelation = IF (Company Fleet=filter(No)) "Service Invoice Header".No.
                            ELSE IF (Company Fleet=filter(true)) "Service Header".No.;

            trigger OnValidate()
            begin
                IF ServInvHead.GET("Invoice No.") THEN BEGIN
                    VALIDATE("Customer No.", ServInvHead."Customer No.");
                    VALIDATE(""Invoice Date"", ServInvHead."Posting Date");
                    "Job Card No." := ServInvHead."Order No.";
                END;
            end; */
        }
        field(3; "Invoice Date"; Date)
        {
            ...
            /* trigger OnValidate()
            begin
                ""Call Due Date"" := CALCDATE('2D', ""Invoice Date"");
        ...
        field(92; Done; Boolean)
        {
            ...
            begin
                Score := "Work Completed the First Time" + "Treatment Reception/Delivery" + "Waitting Time at Reception"
                  + "Service Accuracy" + "Was the Veh. Finished On Time" + "Work/Est. Explanation Recept" + "Standard Expectations" + "Will you Recomend Us to Other";
            end;
        }
        ...
        field(124; "Customer's Feedback"; Text[250])
        {
        }
        field(125; "General Comment"; Text[250])
        {
        }
    }
