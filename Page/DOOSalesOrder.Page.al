...
page 70522 "DOO Sales Order"
{
    ...
    layout
    {
        area(content)
        {
            ...
            part(CustomDocs; "Sales Custom Doc")
            {

                SubPageLink = "Sales Order" = FIELD("No.");
                //UpdatePropagation = Both;

            }
            group(LogisticsApproval)
            {
                Caption = 'Logistics Approval';
                ...
                field("Finance Send to"; Rec."Finance Send to")
                {
                    Caption = 'Send to';
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Select someone in finance department to approve this document.';
                }
            }
            group(FinanceApproval)
            {
                ...
                field("Marketing Send To"; Rec."Marketing Send To")
                {
                    Caption = 'Send To';
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Select someone in marketing department to approve this document.';
                }
            }
            group(MarketingApproval)
            {
                ...
                field("Custom Doc. Availability"; Rec."Custom Doc. Availability")
                {
                    ApplicationArea = Basic, Suite;
                }
            ...
            group(AuditorsConformation)
            {
                ...
                field("Audit Confirmation"; Rec."Audit Confirmation")
                {
                    ApplicationArea = Basic, Suite;
                }
...
#pragma warning restore
        ...
        area(processing)
        {
            ...
            group("Request Approval")
            {
                ...
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Initiate Approval';
                    Image = Approval;

                    trigger OnAction()
                    begin
                        Rec.TESTFIELD("Salesperson Code");
                        Rec.TESTFIELD("Logistics Send to");

                        IF NOT CONFIRM('Are you sure you want to confirm this?', FALSE) THEN
                            Rec."Send for Approval2" := FALSE
                        ELSE
                            Rec.VALIDATE("Send for Approval2", TRUE);
                    end;
                }
        ...
        area(Promoted)
        {
            ...
            group(Category_Category9)
            {
                ...
                actionref(InitiateApproval_Promoted; InitiateApproval)
                {
